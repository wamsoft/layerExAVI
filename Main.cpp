...
protected:
	 ...
	 * @param dwParam1 パラメータ2
	 */
	void waveIn(HWAVEIN hwi, UINT uMsg, DWORD_PTR dwParam1, DWORD_PTR dwParam2) {
		/* イベント処理 */
		switch(uMsg){
		case WIM_DATA:
			{
...
public:
	...
	void openWAV(const tjs_char *filename, int channel, int rate, int bits, int interval) {
		...
		// waveIn を開く
		if (waveInOpen(&hwi, WAVE_MAPPER, &waveForm, (DWORD_PTR)waveInProc, (DWORD_PTR)this, CALLBACK_FUNCTION) != MMSYSERR_NOERROR) {
